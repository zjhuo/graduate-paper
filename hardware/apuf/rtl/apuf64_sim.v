...
module apuf_stage_sim #(
    parameter integer D_TOP_STRAIGHT = 1,
    parameter integer D_BOT_STRAIGHT = 1,
    parameter integer D_TOP_CROSS    = 1,
    parameter integer D_BOT_CROSS    = 1
) (
    input  wire top_in,
    input  wire bot_in,
    ...
    assign #(D_TOP_STRAIGHT) top_straight = top_in;
    assign #(D_BOT_STRAIGHT) bot_straight = bot_in;
    assign #(D_TOP_CROSS)    top_cross    = bot_in;
    assign #(D_BOT_CROSS)    bot_cross    = top_in;

    assign top_out = challenge_bit ? top_cross : top_straight;
    assign bot_out = challenge_bit ? bot_cross : bot_straight;
endmodule
...
module apuf64_sim #(
    ...
    always @(posedge top_path[CHALLENGE_WIDTH] or negedge rst_n) begin
        if (!rst_n) begin
            ...
            noisy_response = bot_path[CHALLENGE_WIDTH];

            if (NOISE_ENABLE != 0) begin
                margin = delay_margin(challenge);
                abs_margin = (margin < 0) ? -margin : margin;

                if ((abs_margin <= NOISE_WINDOW) &&
                    ((noise_hash(challenge, sample_count) % 100) < NOISE_PCT)) begin
                    noisy_response = ~noisy_response;
                end
            end
